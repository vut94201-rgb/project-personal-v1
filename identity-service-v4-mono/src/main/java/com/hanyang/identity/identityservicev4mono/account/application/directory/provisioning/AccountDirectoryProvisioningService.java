...
@ConditionalOnProperty(prefix = "integration.ds389", name = "enabled", havingValue = "true")
@Slf4j
public class AccountDirectoryProvisioningService {
  ...
  public void requestSynchronization(AccountId accountId) {
    ...
    outboxPublisher.publish(
        OUTBOX_AGGREGATE_TYPE, accountId.value().toString(), OUTBOX_EVENT_TYPE, null);
  }
  ...
  public AccountDirectoryReconciliationResult reconcile(AccountId accountId) {
    ...
    if (account == null) {
      return AccountDirectoryReconciliationResult.failed(
          accountId, PROVIDER, "Account not found: " + accountId.value());
    }

    AccountDirectoryProvisioningState syncingState =
        provisioningStateRepository.beginSynchronization(accountId, PROVIDER);
    ...
    try {
      ...
      if (authenticationAllowed(attemptedStatus)
        ...
        directoryAccount =
            accountDirectoryPort.setAuthenticationAllowed(
                latestAccount.getUsername(), authenticationAllowed(latestAccount.getStatus()));
      }
      ...
      AccountDirectoryProvisioningState failedState =
          provisioningStateRepository.failSynchronization(
              accountId, PROVIDER, synchronizedRevision, messageOf(exception));
      log.error(
          "389 DS synchronization failed. accountId={}, username={}",
          accountId.value(),
          account.getUsername(),
          exception);
  ...
  private AccountDirectoryPort.DirectoryAccountSpec directorySpec(Account account) {
    Employee employee =
        employeeRepository
            .findById(account.getEmployeeId())
            .orElseThrow(
                () ->
                    new IllegalStateException(
