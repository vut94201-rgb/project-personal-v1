...
public class AccountRepositoryAdapter implements AccountRepository {
  ...
  public Account save(Account account) {
    ...
    return jpaRepository
        .findById(account.getId().value())
        .map(
            existing -> {
              mapper.updateEntity(account, existing);
              AccountJpaEntity saved = jpaRepository.save(existing);
  ...
  public Optional<Account> findByEmployeeId(EmployeeId employeeId) {
    return jpaRepository.findByEmployeeId(employeeId.value()).map(mapper::toDomain);
  }
  ...
  public boolean existsByEmployeeId(EmployeeId employeeId) {
    return jpaRepository.existsByEmployeeId(employeeId.value());
  }

  @Override
  public Optional<Account> findAccountByEmployeeIdOrUsernameOrEmployeeCode(
