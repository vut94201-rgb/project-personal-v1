  ...
  public void handle(OutboxEvent event) {
    if (!ApplicationProvisioningService.OUTBOX_AGGREGATE_TYPE.equals(event.aggregateType())) {
      throw new IllegalArgumentException(
          "Unexpected aggregate type for application provisioning event: " + event.aggregateType());
    }
    ...
    try {
      ...
      throw new IllegalArgumentException(
          "Invalid application id in outbox event: " + event.aggregateId(), exception);
    }

    ApplicationReconciliationResult result =
        provisioningService.reconcile(new ApplicationId(applicationUuid));
