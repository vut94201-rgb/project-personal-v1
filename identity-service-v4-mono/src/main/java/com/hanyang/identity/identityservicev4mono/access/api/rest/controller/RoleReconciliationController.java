...
public class RoleReconciliationController {
    ...
    public RoleReconciliationResponse reconcile(
            @Valid @RequestBody ReconcileRolesRequest request
    ) {
        List<RoleReconciliationResult> results = request.roleIds().stream()
                .distinct()
                .map(RoleId::new)
